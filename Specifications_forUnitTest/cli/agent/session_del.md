...
| --- | --- | --- | --- | --- | --- | --- |
| TC-001 | 正常系 | 最小有効入力 | --runner_name=enabled_value、--user_name=enabled_value、--session_id=enabled_value。任意パラメータは省略する | RESP_SUCCESS | 正常終了し、結果オブジェクトに warn, success, end が含まれる | output_json が作成され、JSON として読めること、append 指定時は既存内容を保持したまま追記されることを確認する |
...
| TC-004 | 型境界 | runner_name 1文字 | --runner_name に 1 文字値 X を指定する | RESP_SUCCESS | 正常終了し、結果オブジェクトに warn, success, end が含まれる | 最短相当の入力でも分岐や検索条件が崩れないことを確認する |
...
| TC-009 | 型境界 | user_name 1文字 | --user_name に 1 文字値 X を指定する | RESP_SUCCESS | 正常終了し、結果オブジェクトに warn, success, end が含まれる | 最短相当の入力でも分岐や検索条件が崩れないことを確認する |
...
| TC-014 | 型境界 | session_id 1文字 | --session_id に 1 文字値 X を指定する | RESP_SUCCESS | 正常終了し、結果オブジェクトに warn, success, end が含まれる | 最短相当の入力でも分岐や検索条件が崩れないことを確認する |
| TC-015 | 型境界 | session_id 特殊文字 | --session_id に a_日本語 space-_.# を指定する | RESP_SUCCESS | 日本語・空白・記号を含む入力が正しく受理される | 文字化けやエスケープ漏れがないことを確認する |
...
| TC-019 | 結果検証 | 結果キー整合性 | 正常系の代表入力で実行する | RESP_SUCCESS | 結果オブジェクトに warn, success, end が含まれる | 不要なキー欠落や型崩れがないことを確認する |
