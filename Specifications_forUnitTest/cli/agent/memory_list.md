...
| cmd | memory_list |
| クラス | AgentMemoryList |
| モジュール | cmdbox.app.features.cli.cmdbox_agent_memory_list |
| 実装ファイル | /home/ubuntu/cmdbox/cmdbox/app/features/cli/cmdbox_agent_memory_list.py |
| 詳細設計書 | Specifications/cli/agent/memory_list.md |
| 実装上の必須推定 | - |

## 概要

- 日本語: 保存されているMemory設定を一覧表示します。
- 英語: Display a list of saved memory configurations.
...
| TC-003 | 型境界 | kwd 1文字 | --kwd に 1 文字値 X を指定する | RESP_WARN | 正常終了し、結果オブジェクトに success, warn が含まれる | 最短相当の入力でも分岐や検索条件が崩れないことを確認する |
| TC-004 | 型境界 | kwd 特殊文字 | --kwd に a_日本語 space-_.#"'&<> を指定する | RESP_WARN | 日本語・空白・記号を含む入力が正しく受理される | 文字化けやエスケープ漏れがないことを確認する |
