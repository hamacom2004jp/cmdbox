...
| cmd | agent_del |
| クラス | AgentAgentDel |
| モジュール | cmdbox.app.features.cli.cmdbox_agent_agent_del |
| 実装ファイル | /home/ubuntu/cmdbox/cmdbox/app/features/cli/cmdbox_agent_agent_del.py |
| 詳細設計書 | Specifications/cli/agent/agent_del.md |
| 実装上の必須推定 | - |

## 概要

- 日本語: Agent 設定を削除します。
- 英語: Deletes agent configuration.
...
| --- | --- | --- | --- | --- | --- | --- |
| TC-001 | 正常系 | 最小有効入力 | --agent_name=enabled_value。任意パラメータは省略する | RESP_SUCCESS | 正常終了し、結果オブジェクトに warn, success が含まれる | output_json が作成され、JSON として読めること、append 指定時は既存内容を保持したまま追記されることを確認する / 削除処理が成功し、削除対象が残存しないことを確認する |
