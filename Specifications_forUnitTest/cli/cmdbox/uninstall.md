...
| cmd | uninstall |
| クラス | CmdboxUninstall |
| モジュール | cmdbox.app.features.cli.cmdbox_cmdbox_uninstall |
| 実装ファイル | /home/ubuntu/cmdbox/cmdbox/app/features/cli/cmdbox_cmdbox_uninstall.py |
| 詳細設計書 | Specifications/cli/cmdbox/uninstall.md |
| 実装上の必須推定 | - |

## 概要

- 日本語: コンテナをアンインストールします。
- 英語: Uninstalls the container.
...
| TC-007 | 型境界 | install_tag 1文字 | --install_tag に 1 文字値 X を指定する | RESP_WARN | 正常終了し、戻り値とログが期待どおりである | 最短相当の入力でも分岐や検索条件が崩れないことを確認する |
| TC-008 | 型境界 | install_tag 特殊文字 | --install_tag に a_日本語 space-_.#"'&<> を指定する | RESP_WARN | 日本語・空白・記号を含む入力が正しく受理される | 文字化けやエスケープ漏れがないことを確認する |
