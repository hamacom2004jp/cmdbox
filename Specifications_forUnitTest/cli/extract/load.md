...
| cmd | load |
| クラス | ExtractLoad |
| モジュール | cmdbox.app.features.cli.cmdbox_extract_load |
| 実装ファイル | F:/devenv/cmdbox/cmdbox/app/features/cli/cmdbox_extract_load.py |
| 詳細設計書 | Specifications/cli/extract/load.md |
| 実装上の必須推定 | extract_name |

## 概要

- 日本語: 指定されたファイルからテキストを抽出する設定を読み込みます。
- 英語: Loads settings for extracting text from the specified file.
...
| --- | --- | --- | --- | --- | --- | --- |
| TC-001 | 正常系 | 最小有効入力 | --extract_name=enabled_value。任意パラメータは省略する | RESP_SUCCESS | 正常終了し、結果オブジェクトに warn, success が含まれる | output_json が作成され、JSON として読めること、append 指定時は既存内容を保持したまま追記されることを確認する |
