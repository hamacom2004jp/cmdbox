...
| --extract_name | 文字列 | はい | いいえ | いいえ | None | - | ロードする抽出設定の名前を指定します。 |
...
- 結果キー候補: warn
- 処理フロー:
  - 条件 not hasattr(args, 'extract_name') or args.extract_name is None を満たす場合は早期終了し、RESP_WARN。結果キー: warn
  - 条件 not re.match('^[\\w\\-]+$', args.extract_name) を満たす場合は早期終了し、RESP_WARN。結果キー: warn
...
- 実装ファイル: F:/devenv/cmdbox/cmdbox/app/features/cli/cmdbox_extract_load.py
- apprun 実装元: ExtractLoad
- svrun 実装元: ExtractLoad
