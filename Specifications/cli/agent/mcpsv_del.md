...
| --mcpserver_name | 文字列 | はい | いいえ | いいえ | None | - | 削除するMCPサーバ設定の名前を指定します。 |
...
- 結果キー候補: warn
- 処理フロー:
  - 条件 not getattr(args, 'mcpserver_name', None) を満たす場合は早期終了し、RESP_WARN。結果キー: warn
...
- 結果キー候補: success, warn
- 処理フロー:
  - 例外処理を伴って処理する。主な呼出: json.loads, payload.get, configure_path.unlink, dict, redis_cli.rpush, convert.b64str2str
...
- 実装ファイル: F:/devenv/cmdbox/cmdbox/app/features/cli/cmdbox_agent_mcpsv_del.py
- apprun 実装元: AgentMcpDel
- svrun 実装元: AgentMcpDel
