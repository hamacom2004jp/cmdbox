...
list_pipe_func_then = () => {
    ...
    $('#pipe_save').off('click').on('click', async () => {
        ...
        $('.pipe_card').off('click').on('click', pipe_card_func);
        if (result['success']) cmdbox.message(result['success'], true, true);
        else if (result['warn']) cmdbox.message(result['warn'], true, true);
        cmdbox.hide_loading();
    });
    // パイプラインファイルの削除
    $('#pipe_del').off('click').on('click', async () => {
        ...
        const title = pipe_modal.find('[name="title"]').val();
        cmdbox.show_loading();
        if (await cmdbox.confirm(`delete "${title}"?`, true, true)) {
