...
class AgentSkillList(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="保存されているAgentスキルマニフェストを一覧表示します。",
            description_en="Lists saved Agent skill manifests.",
            choice=[
                ...
                dict(opt="kwd", type=Options.T_STR, default=None, required=False, multi=False, hide=False, choice=None,
                    description_ja="検索したい名前を指定します。中間マッチで検索します。",
                    description_en="Specify the name you want to search for. Searches for partial matches."),
            ]
        )
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        try:
            ...
            if agent_dir.exists() and agent_dir.is_dir():
                for p in sorted(agent_dir.glob(f"skill-{kwd}.json")):
                    name = p.name
                    if not name.startswith('skill-') or not name.endswith('.json'):
                        continue
                    results.append(dict(name=name[6:-5], path=str(p)))
            redis_cli.rpush(reskey, dict(success=results))
