...
class ClientFileRead(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self):
        return dict(
            ...
            description_ja="データフォルダ配下のテキストファイルの内容を読み込みます。",
            description_en="Read the content of a text file under the data folder.",
            choice=[
                ...
                dict(opt="svpath", type=Options.T_FILE, default="/", required=True, multi=False, hide=False, choice=None,
                     description_ja="読み込むファイルのパスをデータフォルダ以下で指定します。",
                     description_en="Specify the path of the file to read, relative to the data folder.",
                     test_true={"server":"/file_server/read.txt",
                                "client":"/file_client/read.txt",
                                "current":"/file_current/read.txt"}),
    ...
    def apprun(self, logger:logging.Logger, args:argparse.Namespace, tm:float, pf:List[Dict[str, float]]=[]) -> Tuple[int, Dict[str, Any], Any]:
        ...
        ret = cl.file_read(args.svpath.replace('"',''), encoding=encoding, scope=args.scope,
    ...
    def output_schema(self) -> type:
        class Data(resdata.Data):
            path: Union[str, None] = pydantic.Field(default=None, description="ファイルパス")
            content: Union[str, None] = pydantic.Field(default=None, description="ファイルの内容")
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        st, res_json = f.file_read(svpath, encoding, fwpaths, rjpaths)
