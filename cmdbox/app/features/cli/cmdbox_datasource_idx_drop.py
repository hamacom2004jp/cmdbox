...
class DatasourceIdxDrop(datasource_base.DatasourceBase, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="指定したデータソースのインデックスを削除します。",
            description_en="Drops an index from the specified datasource.",
            choice=[
                ...
                dict(opt="idxname", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="削除するインデックス名を指定します。",
                     description_en="Specify the index name to drop."),
                dict(opt="if_exists", type=Options.T_BOOL, default=False, required=False, multi=False, hide=False, choice=[True, False],
                     description_ja="インデックスが存在しない場合にエラーを出さないようにするかどうかを指定します。",
                     description_en="Specify whether to suppress an error if the index does not exist."),
            ]
        )
