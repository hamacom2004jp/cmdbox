...
class ExtractDel(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        ...
        return dict(
            ...
            description_ja="抽出設定を削除します。",
            description_en="Delete the extraction configuration.",
            choice=[
                ...
                dict(opt="extract_name", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="削除する抽出設定の名前を指定します。",
                     description_en="Specify the name of the extraction configuration to delete."),
            ]
        )
    ...
    def apprun(self, logger: logging.Logger, args: argparse.Namespace, tm: float, pf: List[Dict[str, float]] = []) -> Tuple[int, Dict[str, Any], Any]:
        ...
        payload = dict(extract_name=args.extract_name)
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        try:
            ...
            configure_path = data_dir / ".agent" / f"extract-{extract_name}.json"
            if not configure_path.exists():
                msg = dict(warn=f"Specified Extract configuration '{extract_name}' not found on server at '{str(configure_path)}'.")
                redis_cli.rpush(reskey, msg)
                return self.RESP_WARN

            configure_path.unlink()
            msg = dict(success=f"Extract configuration '{extract_name}' deleted from '{str(configure_path)}'.")
            redis_cli.rpush(reskey, msg)
