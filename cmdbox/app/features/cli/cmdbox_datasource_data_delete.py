...
class DatasourceDataDelete(datasource_base.DatasourceBase, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="指定したデータソースのテーブルからレコードを削除します。",
            description_en="Deletes records from a table of the specified datasource.",
            choice=[
                ...
                dict(opt="tblname", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="削除対象テーブル名を指定します。",
                ...
                dict(opt="where_data", type=Options.T_DICT, default=None, required=False, multi=True, hide=False, choice=None,
                     description_ja="WHERE条件のカラム名と値を key=value 形式で指定します（AND結合）。複数条件は繰り返し指定します。省略時は全件削除となります。",
                     description_en="Specify WHERE condition column names and values in key=value format (AND join). Repeat for multiple conditions. If omitted, all rows are deleted."),
            ]
        )
    ...
    def apprun(self, logger: logging.Logger, args: argparse.Namespace, tm: float, pf: List[Dict[str, float]] = []) -> Tuple[int, Dict[str, Any], Any]:
        payload = dict(dsname=args.dsname, schema=args.schema if hasattr(args, 'schema') else None, tblname=args.tblname, where_data=args.where_data)
        try:
