...
class CmdboxPgSQLUninstall(cmdbox_base.CmdboxBase):
    ...
    def get_option(self):
        ...
        opt['description_ja'] = "PostgreSQLサーバーをアンインストールします。"
        opt['description_en'] = "Uninstalls the PostgreSQL server."
        opt['choice'] = [
            dict(opt="install_pgsqlver", type=Options.T_STR, default="18", required=False, multi=False, hide=False, choice=None,
    ...
    def apprun(self, logger:logging.Logger, args:argparse.Namespace, tm:float, pf:List[Dict[str, float]]=[]) -> Tuple[int, Dict[str, Any], Any]:
        ...
        """
        ret = self.uninstall(logger, args.compose_path, 'pgsql', f"{args.install_pgsqlver}{f'_{args.install_tag}' if args.install_tag else ''}")
