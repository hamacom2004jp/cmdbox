...
class WebGroupList(feature.UnsupportEdgeFeature, validator.Validator):
    ...
    def get_option(self):
        ...
        return dict(
            ...
            description_ja="Webモードのグループ一覧を取得します。",
            description_en="Get a list of groups in Web mode.",
            choice=[
                ...
                dict(opt="ret_hidden", type=Options.T_BOOL, default=False, required=False, multi=False, hide=False, choice=[True, False],
                     description_ja="hidden設定が有効化されているグループを返すかどうかを指定します。省略時は False です。",
                     description_en="Specify whether to return groups with the hidden setting enabled. If omitted, it is False."),
            ]
        )

    @cache.apprun_cache(exclude_fn=lambda args: hasattr(args,'group_name') and args.group_name)
    ...
    def apprun(self, logger:logging.Logger, args:argparse.Namespace, tm:float, pf:List[Dict[str, float]]=[]) -> Tuple[int, Dict[str, Any], Any]:
        ...
        try:
            ...
            groups = w.group_list(args.group_name, ret_hidden=args.ret_hidden)
    ...
    def output_schema(self) -> type:
        class GroupRecord(resdata.Base):
            ...
            buildin: Union[bool, None] = pydantic.Field(default=None, description="ビルトイングループかどうか")
            hidden: Union[bool, None] = pydantic.Field(default=None, description="hidden設定が有効化されているかどうか")
            readonly: Union[bool, None] = pydantic.Field(default=None, description="readonly設定が有効化されているかどうか")
