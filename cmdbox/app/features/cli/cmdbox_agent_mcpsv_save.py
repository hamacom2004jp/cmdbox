...
class AgentMcpSave(feature.OneshotResultEdgeFeature, validator.Validator, limiter.LimitedFeature):
        ...
        return dict(
            ...
            choice=[
                ...
                dict(opt="mcpserver_apikey", type=Options.T_PASSWD, default=None, required=False, multi=False, hide=False, choice=None,
                    description_ja="A2A Server起動（AgentCard作成）時のリモートMCPサーバーのAPI Keyを指定します。 `mcpserver_delegated_auth` が無効な場合は、MCP実行時に使用も使用されます。",
                    description_en="Specify the API key for the remote MCP server when starting the A2A Server (creating an AgentCard). If `mcpserver_delegated_auth` is disabled, this key is also used when running MCP.",),
