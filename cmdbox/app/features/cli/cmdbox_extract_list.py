...
class ExtractList(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        ...
        return dict(
            ...
            description_ja="保存されている抽出設定を一覧表示します。",
            description_en="Display a list of saved extraction settings.",
            choice=[
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        try:
            ...
            if agent_dir.exists() and agent_dir.is_dir():
                paths = agent_dir.glob(f"extract-{kwd}.json")
                for p in sorted(paths):
                    name = p.name
                    if not name.startswith('extract-') or not name.endswith('.json'):
                        continue
                    results.append(dict(name=name[8:-5], path=str(p)))
