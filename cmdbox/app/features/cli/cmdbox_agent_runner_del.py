...
class CmdAgentRunnerDel(feature.OneshotResultEdgeFeature):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            nouse_webmode=False,
            use_agent=True,
            description_ja="Runner 設定を削除します。",
            description_en="Deletes runner configuration.",
            choice=[
                ...
                dict(opt="runner_name", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="削除するRunner設定の名前を指定します。",
                     description_en="Specify the name of the runner configuration to delete."),
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        try:
            ...
            if not configure_path.exists():
                msg = dict(warn=f"Specified runner configuration '{name}' not found on server at '{str(configure_path)}'.")
                redis_cli.rpush(reskey, msg)
                return self.RESP_WARN

            configure_path.unlink()
            msg = dict(success=f"Runner configuration '{name}' deleted from '{str(configure_path)}'.")
            redis_cli.rpush(reskey, msg)
