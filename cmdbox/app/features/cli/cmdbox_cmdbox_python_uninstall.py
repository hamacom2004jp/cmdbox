...
class CmdboxPythonUninstall(feature.OneshotEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="Pythonサンドボックス用Dockerコンテナとイメージを削除します。",
            description_en="Removes the Docker container and image for the Python sandbox.",
            choice=[]
        )
    ...
    def apprun(self, logger: logging.Logger, args: argparse.Namespace, tm: float, pf: List[Dict[str, float]] = []) -> Tuple[int, Dict[str, Any], Any]:
        ...
        try:
            ...
            cmd = f"docker compose -f {compose_path} down --rmi all"
            returncode, _, _cmd = common.cmd(f"{cmd}", logger, slise=-1)
            if returncode != 0:
                ret = dict(warn=f"Failed to remove sandbox: {_cmd[:500]}")
            else:
                ret = dict(success=f"Sandbox removed using {compose_path}.: {_cmd[:500]}")
        except Exception as e:
            ret = dict(warn=f"Failed to remove image: {e}")
        finally:
