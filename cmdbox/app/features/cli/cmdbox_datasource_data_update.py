...
class DatasourceDataUpdate(datasource_base.DatasourceBase, validator.Validator, limiter.LimitedFeature):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="指定したデータソースのテーブルのレコードを更新します。",
            description_en="Updates records in a table of the specified datasource.",
            choice=[
                ...
                dict(opt="tblname", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="更新対象テーブル名を指定します。",
                     description_en="Specify the target table name."),
                dict(opt="update_data", type=Options.T_DICT, default=None, required=True, multi=True, hide=False, choice=None,
                     description_ja="更新するカラム名と値を key=value 形式で指定します。複数のカラムを指定する場合は繰り返し指定します。",
                     description_en="Specify the column names and values to update in key=value format. Repeat for multiple columns."),
