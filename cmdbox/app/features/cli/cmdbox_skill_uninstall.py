...
class SkillUninstall(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="インストール済みAgent Skillsをアンインストールします。",
            description_en="Uninstall an installed Agent Skill.",
            choice=[
                ...
                dict(opt="skill_name", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                     description_ja="アンインストールするスキル名を指定します。",
                     description_en="Specify skill name to uninstall."),
            ]
        )
    ...
    def svrun(self, data_dir: Path, logger: logging.Logger, redis_cli: redis_client.RedisClient, msg: List[str],
        ...
        try:
            ...
            skill_dir = data_dir / '.skills' / str(skill_name)
            if not skill_dir.exists() or not skill_dir.is_dir():
                redis_cli.rpush(reskey, dict(warn=f"Skill '{skill_name}' not found at '{skill_dir}'."))
                return self.RESP_WARN

            shutil.rmtree(skill_dir)
            ret = dict(success=dict(data=f"Skill '{skill_name}' uninstalled from '{skill_dir}'."))
            logger.info(f"Uninstalled skill '{skill_name}' from '{skill_dir}'.")
