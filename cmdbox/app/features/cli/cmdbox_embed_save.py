...
class EmbedSave(feature.OneshotResultEdgeFeature, validator.Validator, limiter.LimitedFeature):
    ...
    def svrun_registrations(self, data_dir, logger, opt, msg):
        embed_dir = data_dir / '.agent'
        count = 0
        if embed_dir.exists() and embed_dir.is_dir():
            paths = embed_dir.glob(f"embed-*.json")
            for p in sorted(paths):
                name = p.name
                if not name.startswith('embed-') or not name.endswith('.json'):
                    continue
                count += 1
        return count
