...
class AgentMemoryList(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        ...
        return dict(
            ...
            description_ja="保存されているMemory設定を一覧表示します。",
            description_en="Display a list of saved memory configurations.",
            choice=[
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        try:
            ...
            if agent_dir.exists() and agent_dir.is_dir():
                paths = agent_dir.glob(f"memory-{kwd}.json")
                for p in sorted(paths):
                    name = p.name
                    if not name.startswith('memory-') or not name.endswith('.json'):
                        continue
