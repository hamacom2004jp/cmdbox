...
class AgentSkillLoad(feature.OneshotResultEdgeFeature, validator.Validator):
    ...
    def get_option(self) -> Dict[str, Any]:
        return dict(
            ...
            description_ja="Agentスキルマニフェストを読み込みます。",
            description_en="Loads an Agent skill manifest.",
            choice=[
                ...
                dict(opt="skill_name", type=Options.T_STR, default=None, required=True, multi=False, hide=False, choice=None,
                    description_ja="読み込むスキルの名前を指定します。",
                    description_en="Specify the name of the skill to load."),
    ...
    def apprun(self, logger: logging.Logger, args: argparse.Namespace, tm: float, pf: List[Dict[str, float]] = []) -> Tuple[int, Dict[str, Any], Any]:
        cached = self._cache.get(args.skill_name)
        if cached is not None:
            ret = dict(success=cached)
            common.print_format(ret, args.format, tm, args.output_json, args.output_json_append, pf=pf)
            return self.RESP_SUCCESS, ret, None

        payload = dict(skill_name=args.skill_name)
        ...
        self._cache.set(args.skill_name, ret.get('success', None), args.cache_timeout)
        return self.RESP_SUCCESS, ret, cl

    def output_schema(self) -> type:
        class Data(resdata.Data):
            data: Union[Dict[str, Any], None] = pydantic.Field(default=None, description="スキルマニフェストのデータ")
    ...
    def svrun(self, data_dir:Path, logger:logging.Logger, redis_cli:redis_client.RedisClient, msg:List[str],
        ...
        try:
            ...
            if not skill_path.exists():
                redis_cli.rpush(reskey, dict(warn=f"Skill '{name}' not found."))
                return self.RESP_WARN
            with skill_path.open('r', encoding='utf-8') as f:
                manifest = json.load(f)
            redis_cli.rpush(reskey, dict(success=manifest))
