...
# CmdboxAppはプロセス内シングルトンで、main()内でlogging・言語設定を更新する。
# sys.stdout/sys.stdin差し替えもプロセス全体に効くため、コマンド実行は必ず直列化する。
# 待ち行列は専用executorに持たせ、既定スレッドプールの枯渇を防ぐ。
_CMD_EXECUTOR = concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix="cmdbox_cmd")
...
class ExecCmd(cmdbox_web_load_cmd.LoadCmd):
        ...
        async def exec_sse_cmd(req:Request, res:Response, title:str=None, scope=Depends(signin.create_request_scope)):
            try:
                ...
                async def sse_event_generator(req:Request, res:Response, title:str=None, opt:Dict[str, Any]=None):
                    ...
                    try:
                        while True:
                            output = resqueue.get(timeout=600)
                            if output is None:
                                break
                            res_val = common.to_str(output)
                            yield f"data: {res_val}\n\n"
                    except queue.Empty:
    ...
    async def _preprocess(self, web:Web, app:FastAPI, req:Request, res:Response, title:str) -> Dict[str, Any]:
        """
        コマンド実行前の前処理を行います。リクエストからオプションを抽出し、コマンドのオプション定義とマージします。
    ...
    async def exec_cmd(self, req:Request, res:Response, web:Web, title:str, opt:Dict[str, Any],
                       nothread:bool=False, appcls=None, resqueue:queue.Queue=None) -> List[Dict[str, Any]]:
        """
        コマンドを実行する
        ...
        Args:
            ...
            resqueue (queue.Queue, optional): 結果を格納するキュー. Defaults to None.
        Returns:
            list: コマンド実行結果
        """
        tags = []
        ...
        ap.cl = None
        ap.web = None
        async def _exec_cmd(cmdbox_app:app.CmdBoxApp, title, opt, req:Request, res:Response,
                            nothread=False, resqueue:queue.Queue=None) -> Any:
            try:
                signin.set_request_scope(dict(req=req, res=res, websocket=None, web=web))
                _stdin_body = None
                ...
                try:
                    ...
                    def to_json(o):
                        ...
                        return res_json
                    def run_main_serialized() -> Tuple[int, Any, Any, str]:
                        # stdio差し替えとmain実行を同一スレッドで完結させる。
                        stdin_bkup = sys.stdin
                        stdout_bkup = sys.stdout
                        captured = io.StringIO() if should_capture_stdout else None
                        try:
                            if _stdin_body is not None:
                                sys.stdin = io.BytesIO(_stdin_body)
                            if captured is not None:
                                sys.stdout = captured
                            st, rm, ob = cmdbox_app.main(
                                args_list=[common.chopdq(o) for o in opt_list],
                                file_dict=file_dict,
                                webcall=True,
                                resqueue=resqueue,
                            )
                            cap = '' if captured is None else captured.getvalue().strip()
                            return st, rm, ob, cap
                        finally:
                    ...
                    status, ret_main, obj, captured_output = await loop.run_in_executor(_CMD_EXECUTOR, run_main_serialized)
                ...
                self.put_queue(resqueue, None)
        if nothread:
            return await _exec_cmd(ap, title, opt, req, res, True, resqueue=resqueue)
