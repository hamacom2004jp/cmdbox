...
class Signin(object):
    ...
    def get_groups(self, access_token:str, user:Dict[str, Any]) -> Tuple[List[str], List[int]]:
        ...
        """
        copy_signin_data = copy.deepcopy(self.signin_file_data)
        group_names = self.__class__.parent_group(copy_signin_data, user['groups'])
    ...
    def _check_authheader(self, req:Request, res:Response, signin_file_data:Dict[str, Any], logger:logging.Logger) -> Union[None, RedirectResponse]:
        ...
        group_names = Signin.parent_group(signin_file_data, find_user['groups'])
    ...
    def parent_group(cls, signin_file_data:Dict[str, Any], group_names:List[str], master_groups:List[Dict[str, Any]]=None) -> List[str]:
        ...
        # 子 -> 親 -> 祖先の順で重複なく返す
        name_to_parent = {gr['name']:gr['parent'] for gr in master_groups if 'name' in gr and 'parent' in gr and gr['parent']}
        ordered_groups = []
        seen = set()
        for gn in group_names.copy():
            current = gn
    ...
    def load_groups(cls, signin_file_data:Dict[str, Any], apikey:str, logger:logging.Logger):
        ...
        group_names = cls.parent_group(signin_file_data, find_user['groups'])
