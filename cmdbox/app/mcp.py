...
class ToolList(object):
    ...
    def _create_func_txt(self, func_name:str, mode:str, cmd:str, is_japan:bool, options:Options, title:str='', params={}) -> str:
        ...
        func_txt += f'    """\n'
        func_txt += f'    logger = logging.getLogger("mcpsv")\n'
        ...
        func_txt +=  '    opt = {o["opt"]: o["default"] for o in options.get_cmd_choices("'+mode+'", "'+cmd+'", False)}\n'
        ...
        func_txt += f'    opt["signin_file"] = _web.signin_file\n'
        func_txt += f'    opt["host"] = _web.redis_host\n'
        func_txt += f'    opt["port"] = _web.redis_port\n'
        func_txt += f'    opt["password"] = _web.redis_password\n'
        func_txt += f'    opt["svname"] = _web.svname\n'
        func_txt += f'    opt["client_only"] = _web.client_only\n'
        func_txt += f'    opt["client_data"] = _web.data\n'
        func_txt += f'    signin_data = signin.Signin.load_signin_file(opt.get("signin_file"))\n'
        func_txt += f'    req = scope["req"] if scope["req"] is not None else scope["websocket"]\n'
        func_txt += f'    sign = _web.signin._check_signin(req, scope["res"], signin_data, logger)\n'
        ...
        func_txt += f'    if not opt.get("clmsg_id"):\n'
        func_txt += f'        if "clmsg_id" in req.session["signin"] and req.session["signin"]["clmsg_id"] is not None:\n'
        func_txt += f'            opt["clmsg_id"] = req.session["signin"]["clmsg_id"]\n'
        ...
        func_txt +=  '        logger.info(f"MCP Call mode='+mode+', cmd='+cmd+', {feat.__class__.__name__}#apprun, args={args.__dict__}")\n'
