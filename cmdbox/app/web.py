...
class Web:
    ...
    def __init__(self, logger:logging.Logger, data:Path, appcls=None, ver=None, language:str=None,
        ...
        self.signin = signin.getDefaultInstance(self.logger, self.signin_file, signin_file_data, self.redis_cli,
    ...
    def get_user_apikeys(self, signin_data:Dict[str, Any], user:Dict[str, Any], include_legacy:bool=True) -> Dict[str, str]:
        ...
        if include_legacy and isinstance(user.get('apikeys', None), dict):
            # 既存のsignin_file由来データは、未移行キーの救済としてのみマージする
            for k, v in user['apikeys'].items():
                if k not in apikeys:
                    apikeys[k] = v
        return apikeys
